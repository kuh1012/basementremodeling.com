...
const saveResizedImages = async (requestID, promises, file, sizes, output) => {
    const { filename, path, destination, originalname  } = file;
    const originalNameNoExtension = originalname.split('.')[0];
    const collectPromises = async ({ 0: width, 1: height, 2: qualityValue }) => {
        ...
        const name = destination + requestID + `/` + originalNameNoExtension.split(' ').join('_') + sizePrefix;
...
const saveOriginalImage = async (requestID, promises, file, output) => {
    const { filename, path, destination, originalname } = file;
    const originalNameNoExtension = originalname.split('.')[0];
    const name = destination + requestID + `/` + originalNameNoExtension.split(' ').join('_');
    const size = {};
...
const saveImages = async (images, files, requestID, removeFolder = true) => {
    ...
    for ( const { name, sizes, output } of images ) {
        ...
        for (const file of files[name]) {
            ...
            const DBFilename = '/' + file.destination + requestID + `/` + file.originalname.split('.')[0].split(' ').join('_');
