...
const {
    requestRedirectsData, deleteRedirectRule, addRedirectRule, updateRedirectRule
} = require('../../models/redirects.model');
...
router.get(`/redirects`, async (request, response, next) => {
    
    request.data['layout'] = `admin`;
    request.data['isAdminRedirectRules'] = true;
    request.data['isHeaderHidden'] = true;
    const content = requestContent(await Promise.all([
        requestRedirectsData()
    ]));
    const data = { ...request.data, ...content };
    const template = `admin/settings/redirects/redirects.admin.hbs`;
    response.render(template, data);
});


router.post(`/redirects/add`, formParser.none(), async (request, response, next) => {
    
    const responseData = await addRedirectRule(request.body);
    return response.json(responseData);
});

router.post(`/redirects/edit`, formParser.none(), async (request, response, next) => {
    
    const responseData = await updateRedirectRule(request.body);
    return response.json(responseData);
});

router.delete(`/redirects/:codeID`, formParser.none(), async (request, response, next) => {
    
    const { params: { codeID } } = request;
    const responseData = await deleteRedirectRule(codeID);
    return response.json(responseData);
});
