...
export const viewIdeaModal = () => {
    ...
    const swipingSetup = () => {
        ...
        ideaModalWrapper.addEventListener('touchstart', e => {
            touchstartX = e.changedTouches[0].screenX
        })

        ideaModalWrapper.addEventListener('touchend', e => {
    ...
    const setModalData = (responseData) => {
        ...
        swipingSetup();
        window.history.pushState("object or string", "Title", window.location.pathname + "?viewIdea=" + ideaID);
    };
    ...
    const showModalIfNeeded = async () => {
        ...
        const ideaId = urlParams.get('viewIdea');
        if (!ideaId) return;  
        changeModalVisible(modalNode)();
        await requestModalData(ideaId);
        changeDataVisible({ isVisible: true });
