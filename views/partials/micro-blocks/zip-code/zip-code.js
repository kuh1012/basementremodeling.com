...
export const zipCodeButtons = () => {
    const scheduleButtons = [...document.querySelectorAll(`.consultationButtonOld`)];
    scheduleButtons.forEach((scheduleButton) => {
        ...
        const zipCodeField = scheduleWrapper.querySelector(`.zipCodeInput`);
        ...
        const sendZipCodeData = async (event) => {
            ...
            ycbWindow.location = responseData.link + `/?REFERRER=${sessionStorage.getItem(`referrer`)}`;
            // TEMP END
